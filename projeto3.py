...
for b in range(brinquedosDiferentes): # Maior numero que pode ser construido de brinquedos b
    ...
    pacotesQueTemBrinquedos = []
    for p in range(nPacotes):
        if infoBrinquedos[b][0] in (infoPacotes[p][1], infoPacotes[p][2],infoPacotes[p][3]):
            pacotesQueTemBrinquedos.append(infoPacotes[p][0])
    prob += infoBrinquedos[b][0] + lpSum(pacotesQueTemBrinquedos[i] for i in range(len(pacotesQueTemBrinquedos)))<= infoBrinquedos[b][2]
